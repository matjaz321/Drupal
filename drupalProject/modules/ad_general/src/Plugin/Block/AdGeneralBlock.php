...
class AdGeneralBlock extends BlockBase implements BlockPluginInterface
{
  ...
  public function build() {
	...
	if ($node) 
	{
		...
		else if ($interval->format('%a') == 0 && date('H:i:s', $date_timestamp) == date('H:i:s')) { //če je event da enaki dan in če je v izvajanju
	  		$output[] = [ '#markup' => 'The event is in progress.',];
		}
		else if ($interval->format('%a') == 0 && date('H:i:s', $date_timestamp) >= date('H:i:s')) { //če je event da enaki dan in če je v izvajanju
	  		$output[] = [ '#markup' => 'The event will start in a few hours.',];
		}
